  ...
  return (
    <section className="ui-panel p-5 md:p-6">
      <div className="flex flex-col gap-4 md:flex-row md:items-start md:justify-between">
        <p className="max-w-2xl text-sm text-(--text-secondary)">Смотрите, что было создано, завершено и изменено недавно, чтобы не терять контекст движения проекта.</p>
      </div>
      ...
      <div className="mt-6 overflow-hidden rounded-3xl border border-(--border) bg-(--panel)">
        {filteredActivities.length > 0 ? filteredActivities.map((activity) => (
          <article key={activity.id} className="relative overflow-hidden border-b border-(--border) bg-(--panel-elevated) p-4 transition hover:bg-[color-mix(in_srgb,var(--panel-elevated)_76%,var(--panel))] md:p-5 last:border-b-0">
        ...
        )) : <div className="p-5"><EmptyState title="Активности пока нет" description="История появится после изменений в проекте." actionLabel="Показать все события" onAction={() => setFilter('all')} /></div>}
      </div>
    </section>
  )
}