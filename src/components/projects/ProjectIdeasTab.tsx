  ...
  return (
    <section className="ui-panel p-5">
      <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
        <p className="max-w-2xl text-sm text-(--text-muted)">Идеи, гипотезы и следующие шаги, связанные с текущим проектом.</p>
        <button type="button" onClick={onCreateIdea} className="ui-button-accent px-4 py-2.5">Добавить идею</button>
      </div>
            ...
            <article key={idea.id} className="ui-panel-elevated p-4">
              <div className="flex flex-col gap-3 md:flex-row md:items-start md:justify-between">
                <button type="button" onClick={() => onOpenIdea(idea.id)} className="min-w-0 flex-1 text-left">
                  <div className="flex flex-wrap gap-2"><span className="ui-chip">{idea.status}</span>{idea.projectId ? <span className="ui-chip">В проекте</span> : null}{ideaTags[0] ? <span className="ui-chip">#{ideaTags[0]}</span> : null}</div>
                ...
                <div className="flex shrink-0 flex-wrap gap-2">
                  {linkedBlockId ? <button type="button" onClick={() => onOpenWorkspaceBlock(linkedBlockId)} className="ui-button px-3 py-2 text-sm">Открыть в workspace</button> : null}
                  <button type="button" onClick={() => onOpenIdea(idea.id)} className="ui-button px-3 py-2 text-sm">Открыть</button>
                </div>
              </div>
            </article>
          )
