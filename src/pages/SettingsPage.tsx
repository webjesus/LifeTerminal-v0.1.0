  ...
  return (
    <section className="mx-auto w-full max-w-[1100px] space-y-4 md:space-y-6">
      ...
      <div className="grid gap-4 xl:grid-cols-[250px_minmax(0,1fr)] xl:gap-6">
        <aside className="ui-panel h-fit overflow-hidden p-4 xl:sticky xl:top-6">
          ...
          <nav className="ui-filter-scroll mt-4 xl:grid xl:gap-2 xl:overflow-visible xl:px-0">
            {sections.map((section) => {
              ...
              return (
                <a key={section.id} href={`#${section.id}`} className="flex min-h-11 shrink-0 items-center gap-3 rounded-2xl border border-(--border-soft) bg-white px-4 py-3 text-sm font-medium text-(--text-secondary) transition-colors duration-200 hover:border-(--accent-border) hover:text-(--text-primary) xl:min-h-12 xl:w-full">
